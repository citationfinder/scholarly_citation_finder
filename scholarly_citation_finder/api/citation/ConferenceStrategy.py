...
class ConferenceStrategy(CitationFinder):
    ...
    def run(self, publication_limit=None, time_limit=None):
        ...
        conferences = self.__find_conferences()
        self.logger.info('found {} conferences in search set'.format(len(conferences)))
        for conference in conferences:
            conference_publications = self.__find_journal_publications(conference.id)            
    ...
    def __find_conferences(self):
        return Conference.objects.using(self.database_name).filter(publication__in=self.publication_search_set).distinct()
    
    def __find_journal_publications(self, conference_id):
