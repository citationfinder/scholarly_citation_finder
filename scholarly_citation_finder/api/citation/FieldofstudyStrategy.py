...
class FieldofstudyStrategy(CitationFinder):
    ...
    def run(self, publication_limit=None, time_limit=None):
        ...
        fieldofstudies = self.__find_fieldofstudies()
    ...
    def __find_fieldofstudies(self):
        return FieldOfStudy.objects.using(self.database_name).filter(publicationkeyword__publication__in=self.publication_search_set).distinct()
